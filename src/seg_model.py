...
from models import Segmentation_model, loss_dice_coefficient_error
...
class SegModel:
    def __init__(self, data_loader, config, use_wandb):
        ...
        self.seg_model_to_save = Segmentation_model(self.img_shape, self.gf).build()
        ...
        # seg = self.Segmentation_model(input_input)
        # self.discriminator.trainable = False
        # valid = self.discriminator([fake_img, input_input])

        # with tf.device('/cpu:0'):
        # self.seg_model = Model(inputs=[input_input], outputs=[seg])
        num_gpu = len(K.tensorflow_backend._get_available_gpus())
        if num_gpu > 1:
            self.seg_model = multi_gpu_model(self.seg_model_to_save, gpus=num_gpu)

        # self.combined.compile(loss=['mse', 'mae'],
        #                       optimizer=self.optimizer_G,
        #                       loss_weights=[config['LOSS_WEIGHT_DISC'],
        #                                     config['LOSS_WEIGHT_GEN']])
        self.seg_model.compile(loss=loss_dice_coefficient_error,
                              optimizer=self.optimizer_G)
    ...
    def train(self):
        ...
        while self.step < max_iter:
            for targets, inputs in self.data_loader.get_random_batch(batch_size):
                #  ---------- Train Discriminator -----------
                # pred = self.seg_model.predict(inputs)
                ...
                # ----------- Train Generator -----------
                g_loss = self.seg_model.train_on_batch([inputs], [targets])
    ...
    def gen_valid_results(self, step_num, prefix=''):
        ...
        fake_imgs = self.seg_model.predict(inputs)
    ...
    def load_model(self, root_model_path):
        ...
        self.seg_model.load_weights(os.path.join(root_model_path, 'discriminator_weights.hdf5'))
    ...
    def save_model(self):
        ...
        # self.seg_model_to_save.save(str(model_dir+'/segmentation_model.h5'))
    ...
    def test(self):
        ...
        for batch_i, (targets, inputs) in enumerate(self.data_loader.get_iterative_batch(3, stage='test')):
            fake_imgs = self.seg_model_to_save.predict(inputs)
            fig = gen_fig(inputs / self.input_trans,
                          fake_imgs / self.target_trans,
                          targets / self.target_trans)
